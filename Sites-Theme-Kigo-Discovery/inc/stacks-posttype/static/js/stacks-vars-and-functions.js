/*******************************************************************************
 *  Stack areas Vars and functions
 *******************************************************************************/
/**
 * Adds slashes
...
function setup_tree(root) {
    ...
    for (var li in root) {
        ...
        // if it's a location
            ...
            try
            {
                type = root[li].originalName.split('::')[1].trim();
            ...
            content += '<li class="ma-item active" data-name=\'' + data_name + '\' data-type="' + type + '" data-original-name="' + data_original_name + '"><a data-name=\'' + data_name + '\' data-original-name="' + data_original_name + '" class="button button-stackarea ' + type + '">' + data_name + '</a><ol class="ma-list">' + innerListItems + '</ol></li>';
        }
    }
...
function reset_areas(e) {
    ...
    $.each(properties, function (i, v) {
        ...
        $(v).remove(); //remove old: This is necessary so the Rates can be updated when saving the stack area.
    });
...
function tree_set_alert($dom_object, alert_type) {
    switch (alert_type) {
        case 'name-repeated':
            var tttitle = 'The name of this location appears more than one time in the tree. There cannot be more than one stack areas with the same name. FIX THIS BEFORE SAVING! otherwise you will not be allowed to.';
...
function remove_all_edit_name()
{
    ...
    $.each($('.stacks-tree.to-save .button-stackarea'), function (i, v)
    {
...
function get_subarea_form(e)
{
    ...
    var form_html = '';
    //the template select
    form_html += '<h3>Select Template</h3><select class="subarea-template">' + $('select#stack_area_use_landing_page_select').html() + '</select>'; //options
...
function add_subarea_form_fill_buttons()
{
    if (tree_watched)
    {
        //after each button-stackarea
        $.each($('.to-save .button-stackarea'), function (i, v)
        {
    ...
    modal(
            {
                ...
                body: '<h1>Error:</h1><p class="description">The Stack areas tree has repeated names. <br /> Please fix this in order to save and create the landing pages.</p>',
...
function create_new_stack(name, words)
{
    // The parent to append the stack to
    var stack_parent = document.querySelector('.ma-list-parent');

    var new_stack = '<li class="ma-item" data-name="' + name + '" data-original-name="' + words + '" data-type="Custom"><a class="button button-stackarea Custom" data-name="' + name + '" data-type="Custom" data-original-name="' + words + '">' + name + '</a><ol class="ma-list"></ol></li>';

    $(stack_parent).append(new_stack);
}
...
function add_stack_node()
{
    ...
    $('.btn-ok-stack-node').on('click', function (e)
    {
        e.preventDefault();
        // Get modal fields:
        var stack_name = $('.stack-name').val();

        var stack_words = $('.stack-words').val();

        // Create new stack button
        create_new_stack(stack_name, stack_words);
...
function sanitize(thestring)
{
    return thestring.replace(/[^1-9A-Za-z_-]/g,'_').toLowerCase();; 
}