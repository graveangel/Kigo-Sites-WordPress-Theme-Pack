    ...
    function get_ma_items($ma_prop_tree=null)
    {
        ...
        foreach($this->ma_types as $mtype)
        {
            foreach($ma_prop_tree as $name => $branch) // Sorting locations (Country, State, City, Neighborhood)
            {
                ...
                if($type === $mtype)
                {
                    $items .= "<li class=\"ma-item\" data-name=\"$lname\"  data-original-name=\"$name\" data-type=\"$type\"><a class=\"button button-stackarea $type\" data-name=\"$lname\" data-type=\"$type\" data-original-name=\"$name\">$lname</a>";
