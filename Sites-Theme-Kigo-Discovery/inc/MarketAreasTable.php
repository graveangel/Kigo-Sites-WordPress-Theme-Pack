...
class MarketAreasTable extends \WP_Posts_List_Table {
    ...
    static function get_market_areas($per_page = 5, $page_number = 1) {
        ...
        # Condition
        $sql .= " INNER JOIN {$wpdb->prefix}postmeta pm on p.ID = pm.post_id WHERE pm.meta_value LIKE '%page-templates/market-area.php%' AND p.post_status <> 'trash'";
        ...
        if (!empty($_REQUEST['orderby'])) {
            ...
            # Need to get the ones with no parent
            
            $no_parent = [];
            foreach ($mktas as $mkta)
            {
                if(!(bool)$mkta->post_parent)
                    $no_parent[] = $mkta;
            }
            
            # Need to get the tree
            foreach($no_parent as $id => $parent)
            {
                $no_parent[$id]->{'children'} = self::build_hierarchy($parent->ID, $mktas);
            }
            
            # Now I need to flatten the array
            $no_parent_array = json_decode(json_encode($no_parent), true);
            $flat_array = self::flatten_array($no_parent_array, 'children');
            
            # Crop the results
        
            $result = array_slice($flat_array, ( $page_number - 1 ) * $per_page, $per_page);
            # debug($result, true);
        }
