...
class StackAreasController
{
    ...
    private function set_vars_and_metas()
    {
        ...
        if(!is_preview())
        {
            ...
            //json_use_landing
            $this->template_vars['json_use_landing'] = array_key_exists('stack_area_use_landing_page',$metas) ? $metas['stack_area_use_landing_page'][0] : '[]';

            //json_tree
            $this->template_vars['tree'] = array_key_exists('stack_area_props_n_areas',$metas) ? $metas['stack_area_props_n_areas'][0] : '[]';
     
            //pics
            $this->template_vars['pics'] = json_decode($metas['stack_area_photos'][0], true);
        }
        else//if(!empty($_SESSION['stack_area_preview_metas']) && !empty($_SESSION['stack_area_preview_metas'][$this->id]))
        {
            
            //get from session
            $metas = $_SESSION['stack_area_preview_metas'][$this->id];
   
            //json_use_landing
            $this->template_vars['json_use_landing'] = array_key_exists('stack_area_use_landing_page',$metas) ? json_encode($metas['stack_area_use_landing_page']) : '[]';

            //json_tree
            $this->template_vars['tree'] = array_key_exists('stack_area_props_n_areas',$metas) ? json_encode($metas['stack_area_props_n_areas']) : '[]';

            //pics
            $this->template_vars['pics'] = $metas['stack_area_photos'];
    ...
    public function set_template()
    {
        ...
        $template_name          = 'stacks-tmpl';


        //Get the saved param:
        $landing_value          = json_decode(stripcslashes(get_post_meta( $this->id,'stack_area_use_landing_page', true)), true);

        if(is_preview())
        {
            $landing_value =  json_decode(json_encode($_SESSION['stack_area_preview_metas']['stack_area_use_landing_page']),true);
        }
...
//        debug($tree, true);
        ...
        /**
         * Need to iterate until originalName is found
         */
        if(is_array($tree))
            foreach($tree as $item)
            {

                if(array_key_exists('originalName',$item))
                    return true;
            }
        ...
        if(is_array($tree))
            foreach($tree as $branch)
            {
                if(array_key_exists('originalName',$branch))
                {
                    ...
                    $post = get_page_by_title( html_entity_decode($branch['name']) , 'OBJECT', 'stacks' );
    ...
    private function fetch_properties()
    {
        ...
        else
        {
            ...
            setcookie('active-tab','.property-list',time()+3600); // Set the properties tab as active
        }
     ...
     */
    function get_properties_ids($tree=null)
    {
        $tree = is_array($tree) ? $tree : json_decode($this->template_vars['tree'], true);
...
$MAInit = new StackAreasController();
