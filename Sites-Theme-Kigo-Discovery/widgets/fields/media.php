...
foreach($value as $pos => $url){
    $aux .= '<div class="widget-media-item" style="background-image: url('.$url.')"><input type="hidden" name="'.$fi['name'].($multiple ? '[]' : '').'" value="'.$url.'" /><div class="delete" onclick="this.parentElement.remove();">Delete</div></div>';
}
