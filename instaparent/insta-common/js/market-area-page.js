marketAreaPage =
        {
            init: function ()
            {
                if (this.cond())
                {
                    ...
                    //Set background for primary-fill-color
            ...
            bottomReached: function (ob)
            {
                ...
                var distance_bottom_doc_to_bottom_window = ((OBJECT_HEIGHT + OBJECT_TOP) - WINDOW_HEIGHT) - scrolled_height;
                var bottom_distance = -1 * distance_bottom_doc_to_bottom_window;
                    
                  return bottom_distance;
            },
            setMapContainer: function (map_container)
            {
                var header_top = $('.global-wrapper > .header-background');
                var headheight = ($(window).height()/* - $('header').height()*/) + 'px';
                ...
                var bottom_offset = /*$('header').height() + */$('#insta-top-fixed').height();
                var top = 0;


                if (this.scrTop('.market-area-content') <= this.scrTop('#insta-top-fixed') + $('#insta-top-fixed').height())
                {
                    top = ($('#insta-top-fixed').height()) + 'px';
                }
                else
                {
            ...
            cond: function ()
            {
                return document.querySelector('.market-area-page');
            }
        }